  ...
  this.Events=(funcs)=>{
    if (!funcs) throw "empty Events()";
      ...
      for (var i=0;i<elements.length;i++) {
        ...
        for (var els=0;els<elements[i].attributes.length;els++) {
          for (const ev of arr) {
            if (elements[i].hasAttributes("ev"))console.warn("TERM"+" "+ev+" "+"found as attribute in"+" "+elements[i].outerHTML+" "+"...remove to avoid pesistent errors");
          }
          if (elements[i].attributes[els]="@click" && elements[i].getAttribute("@click") in funcs) {
            elements[i].onclick = funcs[elements[i].getAttribute("@click")];
          }
        
        //dblclick
       if (elements[i].attributes[els]="@dblclick" && elements[i].getAttribute("@dblclick") in funcs) {
            element[i].ondblclick = funcs[elements[i].getAttribute("@dblclick")];
          }
   //mouseout  
       if (elements[i].attributes[els]="@mouseout" && elements[i].getAttribute("@mouseout") in funcs) {
            elements[i].onmouseout = funcs[elements[i].getAttribute("@mouseout")];
          }      
    //mouseover      
          if (elements[i].attributes[els]="@mouseover" && elements[i].getAttribute("@mouseover") in funcs) {
            elements[i].onmouseover = funcs[elements[i].getAttribute("@mouseover")];
          }
          
    //focus
      if (elements[i].attributes[els]="@focus" && elements[i].getAttribute("@focus") in funcs) {
            elements[i].onfocus = funcs[elements[i].getAttribute("@focus")];
          }     
   //blur
   
  if (elements[i].attributes[els]="@blur" && elements[i].getAttribute("@blur") in funcs) {
...
if(!hooker.search(hookexp2)) throw Error("incomplete hook enclose found in"+" " + state[s].outerHTML);
...
//checking for hooks as attribute values
 ...
 for(var attrhook=0; attrhook<state[s].attributes.length;attrhook++){
   if(state[s].attributes[attrhook]="g-href" && state[s].getAttribute("g-href") in optstates){
         state[s].href= optstates[state[s].getAttribute("g-href")]; 
   }
  
  //hook for classes
 if(state[s].attributes[attrhook] = "g-class" && state[s].getAttribute("g-class") in optstates){
         state[s].className= optstates[state[s].getAttribute("g-class")];
     }
  //hook for src
  
if(state[s].attributes[attrhook]=":src" && state[s].getAttribute(":src") in optstates){
         state[s].src= optstates[state[s].getAttribute("g-class")] || optstates[state[s].getAttribute(":src")]
     }
  
  
 //hook for ids
 
 if(state[s].attributes[attrhook]="g-id" && state[s].getAttribute("g-id") in optstates){
